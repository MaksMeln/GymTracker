...
final class StartWorkoutController :  GTBaseViewController {
    
//MARK: - PROPERTIES
    private let quickStartLabel = GTLabel(text: ResStartWorkout.String.Labels.quickStart,
                                          font: Resourses.Fonts.helveticaBold(with: 19),
                                          textColor: Resourses.Colors.defaultColor)
    
    private let routinesLabel = GTLabel(text: ResStartWorkout.String.Labels.routines,
                                        font: Resourses.Fonts.helveticaBold(with: 19),
                                        textColor: Resourses.Colors.defaultColor)
    
   private var startEmptyWorkoutButton : GTStartWorkoutButton = {
       let button = GTStartWorkoutButton(with: .startEmptyWorkout)
    ...
    private var newRoutineButton : GTStartWorkoutButton = {
        let button = GTStartWorkoutButton(with: .newRoutineAndExplore)
    ...
    private var exlporeButton : GTStartWorkoutButton = {
        let button = GTStartWorkoutButton(with: .newRoutineAndExplore)
        button.setInfo(ResStartWorkout.String.Buttons.explore, ResStartWorkout.Images.explore)
        return button
    }()
    
    private var myRoutinesButton : GTStartWorkoutButton = {
        let button = GTStartWorkoutButton(with: .myRoutines)
...
extension StartWorkoutController {
    ...
    override func setupViews() {
        view.addSubviews(quickStartLabel, startEmptyWorkoutButton, routinesLabel, newRoutineButton, exlporeButton, myRoutinesButton, myRoutinesTable)
...
// MARK: - HELPERS FUNCCTION
    ...
    @objc func showNewRoutineButton() {
        let newRoutine = ExerciseRoutinesTableView()
...
extension StartWorkoutController : PressButton {
